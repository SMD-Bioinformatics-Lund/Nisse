process MAKE_SCOUT_YAML {
    ...
    input:
    tuple val(meta), path(fraser), path(outrider), path(vcf), path(vcf_tbi), path(junctions), path(junctions_tbi), path(cram), path(cram_crai), path(bigwig), path(peddy_sex_check)
    ...
    produce_yaml.py \\
        ...
        --peddy_sex_check "${tomte_results_dir}/qc/peddy/${peddy_sex_check}" \\
